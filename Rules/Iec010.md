# Iec010 - Field type is ambiguous in IEC and should be explicitly defined with a DataType attribute

**Severity:** ![Suggestion](../images/Suggestion.png) Suggestion

When the type of a field attributed with `[Local]`, `[Output]`, ` [Input]`, `[InOut]`, `[InputPort]` and `[OutputPort]` can be mapped to multiple IEC types a default type is used.
